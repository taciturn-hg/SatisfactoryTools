...
### 5.13 已解决：能量碎片分配超支（设置 2 实际分配 4）

**状态**：已解决（2026-08-16）

**问题**：产出「气体过滤器 15/min」、原料输入「纤维织布 30/min」、能量碎片设置为 2 时，节点详情中能量碎片实际分配总数为 4（气体过滤器 200%×2、铁板 150%×1、铁锭 150%×1），超出预算 2 倍。

**根因**：`applyOverclock` 第 2 轮（降频消除）为铁板/铁锭（rawCount=1.5）各写入 1 碎片对应的 150% 时钟，但按注释设计「不消耗 remaining、由第 4 轮统一扣减」——第 4 轮按 rawCount 降序把剩余 2 碎片全部分给气体过滤器（200%）后 `remaining` 归零提前 break，**不再覆盖**第 2 轮已写入的时钟。第 2 轮的预分配成了预算外分配，总消耗 = 2 + 1 + 1 = 4。

**修复**：删除第 2 轮的独立分配，改为仅标记「不值得消除降频」的节点（跳过区间 0~25%、50~75%）；降频消除与整数节点超频合并到第 4 轮，在同一 `remaining` 池内按 rawCount 降序竞争，每次分配真实扣减预算，跳过区间节点预算归零（保持默认降频）。总碎片消耗严格 ≤ `powerShards`。

**验证**：碎片 0/1/2/3/4/5/6/10 全部 ≤ 预算（4 时饱和：气体过滤器 2 + 铁板 1 + 铁锭 1）；高产量+采矿机叶子、索莫晶体、副产回灌场景预算均被遵守（旧逻辑副产回灌场景 2→3、5→7 超支，修复后 2→2、5→5）；叶子/晶体路径与修复前一致；type-check、oxlint 通过。
