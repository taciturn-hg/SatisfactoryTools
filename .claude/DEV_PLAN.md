...
### 5.9 待处理：重油残渣被用塑料配方展开，副产塑料与真塑料线撞车

**状态**：待处理（2026-08-10 记录）

**问题**：需要「重油残渣」（如无烟火药配方含重油残渣）时，`selectRecipe` 因重油残渣无「主产物原生配方」（只有 `Recipe_Alternate_HeavyOilResidue_C` 是主产物替代配方）回退选 `Recipe_Plastic_C`，创建「名义重油残渣、实际塑料产线」的假节点。副产循环开启时该节点副产的塑料与真塑料线（电路板需求）撞车，导致塑料供需错乱（缺料）。
...
**候选方案**（全局折叠）：等整张图展开完后，在 `applyByproductRecycling` 阶段用全局视角检测「目标物品需求已被其他产线副产物覆盖」的假节点，折叠掉并重连边（假塑料线的主产物需求由自己副产供给，重油残渣需求从真塑料线副产来）。需要证明折叠正确性，逻辑比 A/B 复杂。
