...
- [x] 产线/发电数值显示统一为小数点后 3 位（2026-08-08）：公共 `formatRate` 从 1 位改为 3 位，消除 graphTransformer/FlowNode 两处重复定义，统一导入 `src/lib/recipeOptions.ts`；发电页模板内联 `toFixed` 全部改用 `formatRate`，解决总功率尾随零丢失。内部引擎 `toFixed(4)` 精度不变
- [ ] 深色主题完善
